...
- **Rate limiting/transient errors:** 429 and 529 responses are retried for every operation after the `Retry-After` delay (capped at 60 seconds), or bounded backoff when there is none. The Sync Log and job summary report how many retries these caused and how long they waited. Ambiguous 5xx responses are retried only for safe reads and deterministic updates; creates and appends use reconciliation.
