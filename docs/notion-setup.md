...
Scheduled syncs then run every four hours. The health workflow creates the `sync-failure` label automatically when first needed. Notification delivery remains a GitHub setting, not application email; see [health-monitor.md](health-monitor.md) for the complete scheduled-health and recovery policy.
